...
$('#addBonPrepa').on("click", function (event) {
    event.preventDefault()
    if ($(this).closest('form').valid()) {
        $('#addBonPrepa').hide()
        $('#closeBonPrepa').hide()
        $('#loadBonPrepa').css('display', '')
        $('#loadBonPrepa').css('display', '')
        var idchantier = $('#idchantier').val()
        $('#editor12').val(CKEDITOR.instances['editor12'].getData())
        $('#editor13').val(CKEDITOR.instances['editor13'].getData())
        $.ajax({
            type: 'POST',
            url: '/crm/bon/add/prepa/' + idchantier,
            data: JSON.stringify($('#addBonPrepaForm').serializeObject()),
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function (respdata, textStatus, jqXHR) {
                if (jqXHR.status == '201') {
                    $('#modalBonPrepa').modal('toggle')
                    window.location.href = '/crm/bon/' + respdata['result'] + '/' + idchantier
                }
            },
            error: function (xhr, resp, text) {
...
$('#addBonLivre').on("click", function (event) {
    event.preventDefault()
    if ($(this).closest('form').valid()) {
        $('#addBonLivre').hide()
        $('#closeBonLivre').hide()
        $('#loadBonLivre').css('display', '')
        var idchantier = $('#idchantier').val()
        $('#editor8').val(CKEDITOR.instances['editor8'].getData())
        $('#editor9').val(CKEDITOR.instances['editor9'].getData())
        $.ajax({
            type: 'POST',
            url: '/crm/bon/livre/add/' + idchantier,
            data: JSON.stringify($('#addBonLivreForm').serializeObject()),
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function (respdata, textStatus, jqXHR) {
                if (jqXHR.status == '201') {
...
$('#adduBon').on("click", function (event) {
    ...
    if ($(this).closest('form').valid()) {
        ...
        $.ajax({
            type: 'POST',
            url: '/crm/bon/chanpresta/add/' + idchantier,
            data: JSON.stringify($('#adduBonForm').serializeObject()),
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function (respdata, textStatus, jqXHR) {
                if (jqXHR.status == '201') {
                    $('#modalBon').modal('toggle')
                    window.location.href = '/crm/bon/' + respdata['result'] + '/' + idchantier
                }
            },
            error: function (xhr, resp, text) {
...
$('#addDeplacerBon').on("click", function (event) {
    event.preventDefault()
    const id = $('#idbon').val();
    const idchantier = $('#deplacerBon').val();
    $('#addDeplacerBon').hide()
    $('#closeDeplacer').hide()
    ...
    $('#dataBONPST').DataTable({
        ...
        buttons: [
            {
                ...
                className: 'fred2',
                action: function (e, dt, node, config) {
                    if (false === $("input[class='BONNPST']").prop('checked')) {
                        $("input[class='BONNPST']").prop('checked', true)
                    } else {
                        $("input[class='BONNPST']").prop('checked', false)
                    }
                },
                titleAttr: 'Cocher tout /  Décocher tout',
            },
            {
                ...
                className: 'valider',
                action: function (e, dt, node, config) {
                    $("input:checkbox[class='BONNPST']:checked").each(function () {
                        var idbon = $(this).val()
                        $('#Traitement').modal('show')
                        $.ajax({
                            type: 'GET',
        ...
        rowGroup: {
            startRender: function(rows, group) {
                ...
                percu = $.fn.dataTable.render.number(' ', ',', 2, '').display(percu)
                reste = $.fn.dataTable.render.number(' ', ',', 2, '').display(reste)

                return $('<tr/>').append(
                    '<td class="col-blue bolo700" style="background-color: #dbdfef" colspan="4">' + group + '</td>',
                    '<td class="col-blue bolo700" style="background-color: #dbdfef; text-align:right">' + ttc + '</td>',
                    '<td class="col-blue bolo700" style="background-color: #dbdfef; text-align:right">' + percu + '</td>',
                    '<td class="col-blue bolo700" style="background-color: #dbdfef; text-align:right">' + reste + '</td>',
        ...
        "drawCallback": function (row, data, start, end, display) {
            ...
            let a = api.column(5).data().reduce(function (a, b) {
                b = b.split(' ').join('')
                b = b.split(',').join('.')
                b = parseFloat(b)
                return a + b
            ...
            let b = api.column(6).data().reduce(function (a, b) {
                b = b.split(' ').join('')
                b = b.split(',').join('.')
                b = parseFloat(b)
                return a + b
            ...
            let c = api.column(7).data().reduce(function (a, b) {
                b = b.split(' ').join('')
                b = b.split(',').join('.')
                b = parseFloat(b)
                return a + b
            ...
            api.column(5).footer().innerHTML = $.fn.dataTable.render.number(' ', ',', 2, '').display(a);
            api.column(6).footer().innerHTML = $.fn.dataTable.render.number(' ', ',', 2, '').display(b);
            api.column(7).footer().innerHTML = $.fn.dataTable.render.number(' ', ',', 2, '').display(c);

        }
    })
    $('#dataDNBONSFOURNES').DataTable({
        fixedHeader: {
            header: true,
            footer: true,
        },
        ...
        buttons: [
            {
                ...
                className: 'fred2',
                action: function (e, dt, node, config) {
                    if (false === $("input[class='BONNPST2']").prop('checked')) {
                        $("input[class='BONNPST2']").prop('checked', true)
                    } else {
                        $("input[class='BONNPST2']").prop('checked', false)
                    }
                },
                titleAttr: 'Cocher tout /  Décocher tout',
            },
            {
                ...
                className: 'valider',
                action: function (e, dt, node, config) {
                    $("input:checkbox[class='BONNPST2']:checked").each(function () {
                        var idbon = $(this).val()
                        $('#Traitement').modal('show')
                        $.ajax({
                            type: 'GET',
