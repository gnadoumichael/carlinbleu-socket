        ...
        buttons: [
            {
                ...
                className: 'fred2',
                action: function (e, dt, node, config) {
                    if (false === $("input[class='FILOAD']").prop('checked')) {
                        $("input[class='FILOAD']").prop('checked', true)
                    } else {
                        $("input[class='FILOAD']").prop('checked', false)
                    }
                },
                titleAttr: 'Cocher tout /  Décocher tout',
            },
            {
                ...
                className: 'del',
                action: function (e, dt, node, config) {
                    $("input:checkbox[class='FILOAD']:checked").each(function () {
                        const id = $(this).val();
                        $('#ConfirmDelete').modal('show')
                        $('#accept').click(function () {
                            $('#ConfirmDelete').modal('hide')
...
$('#addFileChantier').on("click", function (event) {
    event.preventDefault()
    if ($(this).closest('form').valid()) {
...
$('#addFileTechnique').on("click", function (event) {
    event.preventDefault()
    if ($(this).closest('form').valid()) {
...
$('#addFileLoading').on("click", function (event) {
    event.preventDefault()
    if ($(this).closest('form').valid()) {
        $('#addFileLoading').hide()
        $('#closeFileLoading').hide()
        $('#loadFileLoading').css('display', '')
        $.ajax({
            type: 'POST',
            url: '/crm/gestion/fichier/loading/add',
            data: new FormData($('#addFileLoadingForm')[0]),
            contentType: false,
            processData: false,
            cache: false,
            dataType: 'json',
...
$('#addFileDesc').on("click", function (event) {
    event.preventDefault()
    if ($(this).closest('form').valid()) {
        $('#addFileDesc').hide()
        $('#closeFileDesc').hide()
