$(document).ready(function() {
    ...
    $('#dataBenef').DataTable({
        ...
        buttons: [{
            ...
            className: 'fred2',
            action: function(e, dt, node, config) {
                if (false === $("input[class='BENEF']").prop('checked')) {
                    $("input[class='BENEF']").prop('checked', true)
                } else {
                    $("input[class='BENEF']").prop('checked', false)
                }
            },
            titleAttr: 'Cocher tout /  Décocher tout',
        },
            {
                ...
                className: 'edit',
                action: function(e, dt, node, config) {
                    $("input:checkbox[class='BENEF']:checked").each(function() {
                        var id = $(this).val()
                        window.location.href =
                            '/crm/materiel/beneficiaire/' + id
                    })
                },
                titleAttr: 'Modifier',
            },
...
$('#addBenef').off().on("click",function(event) {
    if ($(this).closest('form').valid()) {
        ...
        $.ajax({
            type: 'POST',
            url: '/crm/materiel/benef/add',
            data: JSON.stringify($('#addBenefForm').serializeObject()),
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function(respdata, textStatus, jqXHR) {
...
$('#updBenef').click(function(event) {
    event.preventDefault()
    if ($(this).closest('form').valid()) {
        $('#updBenef').hide()
        $('#closeBenef').hide()
        $('#loadBenef').css('display', '')
        const id = $('#idbenef').val();
        $.ajax({
            type: 'PUT',
            url: '/crm/materiel/benef/update/' + id,
            data: JSON.stringify($('#addBenefForm').serializeObject()),
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function(respdata, textStatus, jqXHR) {
