...
$('#updateSer').off().on("click",function(event) {
    event.preventDefault()
    if ($(this).closest('form').valid()) {
        $('#updateSer').hide()
        $('#closeSer').hide()
        ...
        $.ajax({
            type: 'POST',
            url: '/crm/personnel/service/update/' + id,
            data: JSON.stringify($('#addServiceForm').serializeObject()),
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function(respdata, textStatus, jqXHR) {
...
$('#saveDroit').off().on("click",function(event) {
    event.preventDefault()
    const id = $('#idcompte').val();
    if ($(this).closest('form').valid()) {
        $.ajax({
            type: 'POST',
            url: '/crm/parametre/droit/add/'+ id,
            data: JSON.stringify($('#addDroitUserForm').serializeObject()),
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function(respdata, textStatus, jqXHR) {
                location.reload()
...
$('#addEquipe').off().on("click", function (event) {
    event.preventDefault()
    if ($(this).closest('form').valid()) {
        $('#divaddequipe').hide()
