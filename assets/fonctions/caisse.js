$(function() {
    ...
    $('#dataCAISSE').DataTable({
        ...
        columnDefs: [
            ...
            {
                targets: 2,
                render: function(data, type, full) {return moment(new Date(data)).locale('fr-fr').format('DD MMM YYYY');
                }
            }
        ],
        dom: 'Bfrtip',
...
$('#sefone').off().on("change", function (event) {
    ...
    $.ajax({
        ...
        success: function (respdata, textStatus, jqXHR) {
            ...
            let groupe = '<option selected>Veuillez choisir un bon de commande svp !</option>';
                groupe.append($selbon);
            respdata.map((bo) => {
