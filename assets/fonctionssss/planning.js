...
$('#updaSection').on("click",function (event) {
    event.preventDefault()
    if ($(this).closest('form').valid()) {
        $('#updaSection').hide()
        $('#closeSection').hide()
        $('#loadSection').css('display', '')
        const id = $('#idsection').val();
        $.ajax({
            type: 'POST',
            url: '/crm/planning/section/update/' + id,
            data: JSON.stringify($('#addSectionForm').serializeObject()),
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function (respdata, textStatus, jqXHR) {
...
$('#addeTache').on("click",function (event) {
    event.preventDefault()
    if ($(this).closest('form').valid()) {
        $('#addeTache').hide()
        $('#closeTache').hide()
        $('#loadTache').css('display', '')
        $('#editors').val(CKEDITOR.instances['editors'].getData())
        $.ajax({
            type: 'POST',
